...
iptables-save > /backup/iptables_backup_$(date +%Y%m%d_%H%M%S).bak 2>/dev/null || mkdir -p /backup && iptables-save > /backup/iptables_backup_$(date +%Y%m%d_%H%M%S).bak
...
echo "iptables-restore < /backup/iptables_backup_YYYYMMDD_HHMMSS.bak" 